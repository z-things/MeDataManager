...
var dataReduce = function (key, values) {
    ...
    if ("050608070001" === record.type) {
        ...
        values.forEach(function (item) {
            sum += item.data.dis_temp
        });
        record.data.dis_temp = sum / values.length;
        delete record.__v;
    }
    return record;
};
...
function dataManager(conx, uuid, token, configurator) {
    ...
    this.statistics = function () {
        ...
        var command = {
            query: {
                "timestamp": {
                    "$gte": new Date("2017-10-24T05:00:00.000Z"),
                    "$lt": new Date("2017-10-24T06:00:00.000Z")
                }
            },
            map: dataMap,
            reduce: dataReduce
        };
        self.dataModel.mapReduce(command, function (err, dbres) {
            if (err) {
                logger.debug(err);
            }
            else {
                _.forEach(dbres, function (result) {
                    ...
                    var entity = new self.dailyDataModel(result.value);
