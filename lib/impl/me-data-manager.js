...
function dataManager(conx, uuid, token, configurator) {
    ...
    this.dailyReportStatistics = function (deviceType) {
        var self = this;
        ...
        self.dataModel.aggregate([{$match: match}, {$group: group}], function (error, result) {
            if (error) {
                logger.debug(error);
            }
            else {
                ...
                _.forEach(result, function (item) {
                    ...
                    delete item._id;
                    data["data"] = item;
                    var mongooseEntity = new self.dailyDataModel(data);
                    mongooseEntity.save();
                })
            }
        });
    ...
    this.statistics = function () {
        var self = this;
        self.dailyReportStatistics("050608070001");
